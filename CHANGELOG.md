...
- Changed `EmbeddingELModel.add_axioms` to discard the loaded datasets and index dictionaries, so the next read of `class_index_dict` renormalizes the modified ontology, and to rebuild the embedding rows in index-dictionary order rather than `dataset.classes` order. The two differ by the appended auxiliary entities, and rebuilding over `dataset.classes` left the rows misaligned with the names `class_embeddings` reads them back through [#126][i126].
