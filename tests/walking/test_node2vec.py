...
class TestNode2Vec(TestCase):
    ...
    def test_node2vec_raise_error_with_incorrect_types(self):
        ...
        walk_length = 5
        p = "0.1"
        self.assertRaisesRegex(TypeError, "Optional parameter p must be of type int or float", Node2Vec,
                               num_walks, walk_length, p=p)

        num_walks = 10
        walk_length = 5
        q = "0.1"
        self.assertRaisesRegex(TypeError, "Optional parameter q must be of type int or float", Node2Vec,
