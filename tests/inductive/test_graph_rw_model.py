...
class TestRandomWalkModel(TestCase):
    ...
    def test_from_pretrained(self):
        ...
        with self.assertRaisesRegex(
            TypeError,
            "Parameter model must be a string pointing to the Word2Vec model file.",
        ):
            model.from_pretrained(1)

        with self.assertRaisesRegex(
            FileNotFoundError, "Pretrained model path does not exist"
        ):
    ...
    def test_train_after_pretrained(self):
        first_model = RandomWalkPlusW2VModel(
            self.dataset, model_filepath="first_rw_w2v_model"
        )
