...
from tests.datasetFactory import GDADatasetV2
...
class EvaluationModel(th.nn.Module):
    def __init__(self, kge_method, triples_factory, dataset, embedding_size, device):
        ...
        relation_id = triples_factory.relation_to_id[dataset.evaluation_object_property]
...
class TestGDA(TestCase):
    ...
    def test_gda_evaluator(self):
        ...
        num_classes = len(self.dataset.evaluation_classes[1])
        true_auc = auc_from_mr(mr, num_classes)
        diff_auc = abs(auc - true_auc)
        self.assertLess(diff_auc, allowed_diff)

        self.model.evaluate(self.dataset.testing, mode="tail_centric")
        mr = self.model.metrics["mr"]
        auc = self.model.metrics["auc"]
