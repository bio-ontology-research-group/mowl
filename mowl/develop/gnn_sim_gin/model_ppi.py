...
class PPIModel(nn.Module):
    ...
    def __init__(self, num_rels, num_bases, num_nodes, n_hid, dropout):
        ...
        for i in range(n_hid):
            ...
            newLayer = RelGraphConv(self.h_dim, self.h_dim, self.num_rels, "basis", self.num_bases, activation = act, self_loop = True, dropout = dropout)
            self.rgcn_layers.append(newLayer)

        for i in range(n_hid):
            self.rgcn_layers[i].register_forward_hook(self._save_inter_feats(i))
...
#            nn.Dropout()
    ...
    def forward_each(self, g, features, edge_type, norm):
        initial = features.reshape(-1, self.num_nodes)
        x = features 
...
#        self._features[-1] = initial
        ...
        x = th.sum(out_feats, dim = 0)
        x = th.flatten(x).view(-1, self.num_nodes*self.h_dim)
        x = self.net(x)

        mlp = self.net2(initial)
        x_mlp = th.cat([x,mlp], dim = 1)

        final = self.netFinal(x_mlp)
        return F.relu(final)
