...
class CatModel(nn.Module):
    ...
    def __init__(self, num_objects, num_rels, embedding_size, dropout = 0):
        ...
        self.pullback_morphisms = self.pullback2left, self.left2end, self.pullback2right, self.right2end
        ...
        self.embed_pullback = nn.Sequential(
        ...
        self.exist_nets = self.embed_slicer, self.embed_extra, self.embed_right_pullback, self.embed_pullback, self.slicer, self.embed_bigger_prod
    ...
    def forward(self, normal_form, idx, neg = False, margin = 0):
        ...
        if idx == 1:
            ...
            embed_nets = (self.net_object, self.net_rel, self.embed_snd, self.embed_up)
            loss = L.nf4_loss(normal_form, self.product_morphisms, self.pullback_morphisms,  self.exponential_morphisms, embed_nets, self.exist_nets, neg = neg, num_objects = self.num_obj)
        else:
