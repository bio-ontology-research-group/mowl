...
class GNNSimPPI(Model):
    ...
    def load_ppi_data(self):
        ...
        train_df = train_df.iloc[index[:100000]]

        index = np.arange(len(valid_df))
        np.random.seed(seed=0)
        np.random.shuffle(index)
        valid_df = valid_df.iloc[index]#[:8000]]
        
        index = np.arange(len(test_df))
        np.random.seed(seed=0)
        np.random.shuffle(index)
        test_df = test_df.iloc[index]#[:8000]]
...
#        print("feats sizes: ", feat1.shape, feat2.shape, feats.shape)

        for l in self.rgcn_layers:
            x = l(g, feats, edge_type, norm)
