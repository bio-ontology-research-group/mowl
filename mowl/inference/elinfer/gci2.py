...
class GCI2Inference():
    ...
    def infer_subclass(self, subclass_condition = None, property_condition = None, filler_condition = None, axioms_to_filter = None):
        ...
        if not axioms_to_filter is None:
            triples = process_axioms(axioms_to_filter, "taxonomy_rels", taxonomy = False, properties = properties)
            rows, cols = zip(*[((self.property_index_dict[p], self.class_index_dict[d]), subclass_indexemb_indexsc[self.class_index_dict[c]]) for c,p,d in triples if (self.property_index_dict[p], self.class_index_dict[d]) in prop_filler_idx and c in subclasses] )
    ...
    def infer_superclass_property(self, subclass_condition = None, property_condition = None, filler_condition = None, axioms_to_filter = None):
        ...
        if not axioms_to_filter is None:
            triples = process_axioms(axioms_to_filter, "taxonomy_rels", taxonomy = False, properties = properties)
            rows, cols = zip(*[((self.class_index_dict[c],self.class_index_dict[d]), property_indexemb_indexsc[self.property_index_dict[p]]) for c,p,d in triples if (self.class_index_dict[c],self.class_index_dict[d]) in sub_filler_idx and p in properties] )
    ...
    def infer_superclass_filler(self, subclass_condition = None, property_condition = None, filler_condition = None, axioms_to_filter = None):
        ...
        if not axioms_to_filter is None:
            triples = process_axioms(axioms_to_filter, "taxonomy_rels", taxonomy = False, properties = properties)
            rows, cols = zip(*[((self.class_index_dict[c],self.property_index_dict[p]), filler_indexemb_indexsc[self.class_index_dict[d]]) for c,p,d in triples if (self.class_index_dict[c],self.property_index_dict[p]) in sub_prop_idx and d in fillers] )
    ...
    def get_inferences(self, top_k = float("inf"), infer_mode="subclass", subclasses = None, properties = None, fillers = None):
        ...
        for idxA_idxB, missing_entity in tqdm(zipped_idxs):
            ...
            if infer_mode == "filler":
                ...
                sub, prop, filler = self.index_class_dict[idxA], self.index_property_dict[idxB], self.filler_name_indexemb_inverse[self.filler_indexemb_indexsc_inverse[missing_entity]]
            elif infer_mode == "subclass":
                idxA_idxB_inverse = self.prop_filler_idx_inverse
                idxA, idxB = idxA_idxB_inverse[idxA_idxB]
                sub, prop, filler = self.subclass_name_indexemb_inverse[self.subclass_indexemb_indexsc_inverse[missing_entity]], self.index_property_dict[idxA], self.index_class_dict[idxB]
            elif infer_mode == "property":
                idxA_idxB_inverse = self.sub_filler_idx_inverse
                idxA, idxB = idxA_idxB_inverse[idxA_idxB]
                sub, prop, filler = self.index_class_dict[idxA], self.property_name_indexemb_inverse[self.property_indexemb_indexsc_inverse[missing_entity]], self.index_class_dict[idxB]
...
class InferGCI2Dataset(IterableDataset):
    def __init__(self, class_name_indexemb, property_name_indexemb, mixed_indexemb_dict, missing_entity_indexemb_indexsc, mode = "infer_filler"):
        super().__init__()
...
def process_axioms(ontology, projection_method, taxonomy = True, bidirectional_taxonomy = False, properties = None):
    projector = projector_factory(projection_method, taxonomy = taxonomy, bidirectional_taxonomy = bidirectional_taxonomy, relations = properties)
