...
class PPIEvaluatorWithModel():

    """
    Evaluation model for protein-protein interactions
   
    """
    def __init__(self, model, training_set: list, testing_set: list, device = "cpu"):

        
        self.rels = list({r for _,r,_ in training_set})
    ...
    def evaluate(self, show = False):
        ...
        with ck.progressbar(eval_data) as prog_data: 
            for c, r, d in prog_data: 
                ...
                res = self.model_nf4(data).cpu().detach().numpy()                                                                                                                    
