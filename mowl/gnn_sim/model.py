...
class GNNSim(Model):
    def __init__(self,
                 ...
                 epochs,
                 output_model_path,
                 graph_generation_method = "taxonomy", #Default: generate ontology taxonomy
    ...
    class PPIModel(nn.Module):
        ...
        def __init__(self, h_dim, num_rels, num_bases, num_nodes, n_hid, dropout):
            ...
            self.rgcn = self.RGCN(self.h_dim,
                                  self.h_dim, 
                                  self.h_dim, 
                                  self.num_rels, 
                                  self.num_bases,
    ...
    def train(self, n_hid, dropout, lr, num_bases, batch_size, epochs, file_params, checkpoint_dir = None, tuning= False):

        g, annots, prot_idx = self.load_graph_data(file_params)
        ...
        model = self.PPIModel(feat_dim, num_rels, num_bases, num_nodes, n_hid, dropout)
        ...
        train_data = self.GraphDataset(g, train_df, train_labels, annots, prot_idx)
        val_data = self.GraphDataset(g, val_df, val_labels, annots, prot_idx)
    
        train_set_batches = self.get_batches(train_data, batch_size)
        val_set_batches = self.get_batches(val_data, batch_size)
        ...
        for epoch in range(epochs):
            ...
            if not tuning:
                if roc_auc > best_roc_auc:
                    ...
                    th.save(model.state_dict(), self.output_model_path)
    ...
    def evaluate(self, n_hid, dropout, num_bases, batch_size, file_params, model=None):
        ...
        test_data = self.GraphDataset(g, test_df, test_labels, annots, prot_idx)
    
        test_set_batches = self.get_batches(test_data, batch_size)
    ...
    def load_ppi_data(self, file_params):
    ...
    def load_graph_data(self, file_params):
        ...
        generator = gen_factory(self.graph_generation_method)
