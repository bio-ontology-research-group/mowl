...
class ELBEVisualizer(ELVisualizer):
    """2-D visualizer for ELBE (box geometry).

    Each class is drawn as an axis-aligned rectangle. The center is
    ``module.class_embed`` and the half-extents are ``|module.class_offset|``.

    :param model: A trained :class:`~mowl.models.ELBE` model with ``embed_dim=2``.
    """
